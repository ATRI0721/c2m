...
export const localMcpApi = {
  ...
  /**
   * Tools are not derivable from `servers.json` alone. In local-file mode we
   * return an empty set so the UI can still render.
   */
  ...
  async updateServers(next: MCPServersResponse): Promise<void> {
    // Preserve extra fields like `env` by merging with existing file.
    const file = await getServersFile();
    const existing = file.mcpServers ?? {};
    const mcpServers: NonNullable<LocalMcpServersFile['mcpServers']> = {};
    ...
    await saveServersFile({ ...file, mcpServers });
  },
  ...
  async importServersFromJson(
    ...
    // Supported formats:
    // 1) { mcpServers: { [name]: {command,args,description,env,enabled} } }
    // 2) { name, command, args?, description?, env?, enabled? } (single server)
